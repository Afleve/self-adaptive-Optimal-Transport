...
from models.CONCH.conch.open_clip_custom import tokenize, get_tokenizer
...
def clip_classifier(classnames, template, clip_model, model_name, reduce='mean', gpt=False, wordnet_dict=None):
    with torch.no_grad():
        clip_weights = []
        ensemble_conch_weights = []
        ensemble_plip_weights = []
        if wordnet_dict is not None:
            ...
            for classname in classnames:
                ...
                # CONCH
                    ...
                    tokenizer =  XLMRobertaTokenizer("models/MUSK/musk/models/tokenizer.spm")
                    text_ids = []
                    paddings = []
                    for txt in texts:
                    ...
                    with torch.no_grad():
                        class_embeddings = clip_model(
                            text_description=text_ids.cuda(),
                            padding_mask=paddings.cuda(),
...
def pre_load_features(cfg, split, clip_model, model_name, loader, n_views=1):
    ...
    if cfg['load_pre_feat'] == False:
                ...
                for i, (images, target) in enumerate(tqdm(loader)):
                    if n_views == 1:
                        ...
                        if model_name == "PLIP":
                            ...
                            image_features = clip_model(
                            image = images.float(),
                            with_head=True, # We only use the retrieval head for image-text retrieval tasks.
                            out_norm=True,
                            ms_aug=False  # by default it is False, `image_embeddings` will be 1024-dim; if True, it will be 2048-dim.
                            )[0]
                        else:
                            image_features = clip_model.encode_image(images)
        ...
        torch.save(features, cfg['cache_dir'] + "/" + split + f"{model_name}_f.pt")
        torch.save(labels, cfg['cache_dir'] + "/" + split + f"{model_name}_l.pt")
   
    else:
        
        features = torch.load(cfg['cache_dir'] + "/" + split + f"{model_name}_f.pt")
        labels = torch.load(cfg['cache_dir'] + "/" + split + f"{model_name}_l.pt")
