...
import torch
import argparse
from TransCLIP_solver.TransCLIP_inference import *
...
def parse_arguments():
    ...
    # parser.add_argument('--model_names', nargs='+', default=["CLIP", "GeoRSCLIP"], help='List of model names')
    # parser.add_argument('--model_architectures', nargs='+', default=["RN50", "ViT-B-32", "ViT-L-14", "ViT-H-14"], help='List of model architectures')
    parser.add_argument('--model_architectures', nargs='+', default=["ViT-B-32"], help='List of model architectures')
    parser.add_argument('--dataset_names', nargs='+', default=["AID", "EuroSAT", "MLRSNet", "OPTIMAL31", "PatternNet", "RESISC45", "RSC11", "RSICB128", "RSICB256", "WHURS19"], help='List of dataset names "RSICB128",')
    # parser.add_argument('--dataset_names', nargs='+', default=["EuroSAT", "OPTIMAL31", "PatternNet", "RESISC45", "RSC11","RSICB256", "WHURS19"], help='List of dataset names')
...
if __name__ == "__main__":
    ...
    for dataset_name in args.dataset_names:
        ...
        for model_name in args.model_names:
            for model_architecture in args.model_architectures:
                for text_prompt in text_prompts:
                    test_features, test_labels, clip_prototypes = extract_features(dataset_name, model_name, model_architecture, text_prompt, averageprompt)
                    features_dict[dataset_name][f"{model_name}_{model_architecture}_{text_prompt}"] = {
    ...
    for dataset_name in args.dataset_names:
        temp_res = []
        for key, value in features_dict[dataset_name].items():
            print(f"Processing: {key}")
            test_features, test_labels, clip_prototypes = value["test_features"], value["test_labels"], value["clip_prototypes"]
            clip_logits = prepare_objects(test_features, test_labels, clip_prototypes)
            
            acc = cls_acc(clip_logits.cpu(), test_labels)
            temp_res.append(acc)
            # z, test_acc_at_best_val, acc_base_zs, acc = TransCLIP_solver(None, None, None, None, test_features, test_labels, clip_prototypes)
            # temp_res.append(acc)

            features_dict[dataset_name][key]["clip_logits"] = clip_logits
        res.append(temp_res)
    ...
    for i, dataset_name in enumerate(args.dataset_names):
        ...
        test_labels = [features_dict[dataset_name][key]["test_labels"] for key in features_dict[dataset_name].keys()][0]
        clip_logits = [features_dict[dataset_name][key]["clip_logits"].float() for key in features_dict[dataset_name].keys()]
        print("Process Dataset: ", dataset_name)
        z = OptimalTrans(features, test_labels, clip_logits)
        acc = cls_acc(z.cpu(), test_labels)
        res[i].append(acc)
        # res[i].append(transclip_res[i])
        res[i] = [round(x, 2) for x in res[i]]
        print(res[i])